...
| **New enquiry analysis** — "analyse", "assess", "review", "look at", "what should I do with", "how should we handle", "break down", "help me respond to" **any client enquiry/email/quote request** | `analyze_enquiry(enquiry="...")` | **MANDATORY when the user asks to analyse/assess/review an enquiry or pastes a client email.** Runs search_crm + retrieve_similar_decisions + search_wiki internally + loads the rate card + classifies job size (small/mid/large) + picks a size-appropriate template + synthesises a brief via Sonnet. Return the output ESSENTIALLY VERBATIM with only a one-line intro — do NOT re-write, re-structure, or re-synthesise the brief. Do NOT call search_crm / retrieve_similar_decisions / search_wiki separately and build your own response; the analyzer already does that and the combined output is tighter and rate-card-grounded. The word "analyse" / "analyze" / "assess" / "review" / "look at this enquiry" / "how do I handle" all trigger this tool. |
