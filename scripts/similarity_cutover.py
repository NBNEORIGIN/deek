#!/usr/bin/env python3
"""Triage Phase D cutover — flip DEEK_SIMILARITY_SHADOW to false.

Scheduled one-shot via cron for 2026-05-05 09:00 UTC. Checks that
similarity has run enough to be worth surfacing, then:

  1. Updates /opt/nbne/deek/deploy/.env — DEEK_SIMILARITY_SHADOW=false
  2. Restarts deek-api so the env takes effect
  3. Writes a cutover record to data/similarity_cutover.jsonl

Mirrors scripts/impressions_cutover.py and scripts/crosslink_cutover.py.

Safety gates:
    - At least 20 debug rows logged in triage_similarity_debug
    - Span of at least 3 days between first and last row
    - Env file exists and is writable
    - deek-api container is running

Usage:
    python scripts/similarity_cutover.py              # safe mode
    python scripts/similarity_cutover.py --dry-run    # report only
    python scripts/similarity_cutover.py --force      # skip gates
"""
...
CUTOVER_LOG = Path(os.getenv(
    'DEEK_SIMILARITY_CUTOVER_LOG',
    str(REPO_ROOT / 'data' / 'similarity_cutover.jsonl'),
))
...
log = logging.getLogger('similarity_cutover')


def read_shadow_stats() -> dict:
    """Count rows + span in cairn_intel.triage_similarity_debug."""
    import psycopg2
    db_url = os.getenv('DATABASE_URL', '')
    if not db_url:
        return {'records': 0, 'span_hours': 0, 'useful_count': 0,
            ...
            with conn.cursor() as cur:
                cur.execute(
                    """SELECT COUNT(*),
                              ...
                              COUNT(*) FILTER (WHERE useful_index IS NOT NULL)
...
def flip_env_file(env_file: Path) -> bool:
    ...
    if not replaced:
        if out and not out[-1].endswith('\n'):
            out.append('\n')
        out.append('\n# Triage similarity — Phase D cutover (auto-applied)\n')
        out.append(wanted)
    tmp = env_file.with_suffix(env_file.suffix + '.simcut-tmp')
    try:
...
def main() -> int:
    ...
    log.info('--- similarity shadow stats ---')
    ...
    record['cutover_applied'] = True
    write_cutover_record(record)
    log.info('CUTOVER COMPLETE — DEEK_SIMILARITY_SHADOW=false')
    return 0
