...
def _load_source(
    ...
    if name == 'material_prices':
        from .sources.material_prices import MaterialPricesSource
        return MaterialPricesSource()
    raise ValueError(
...
KNOWN_SOURCES = {
    ...
    'material_prices': {'needs_llm': True,  'needs_data_file': None, 'needs_ledger_db': False},
}
...
def preflight(sources: list[str], data_dir: Path, commit_mode: bool) -> list[str]:
    ...
    built_sources = {'synthetic', 'disputes', 'b2b_quotes', 'principles', 'xero', 'amazon', 'crm_lessons', 'crm_reflection', 'material_prices'}
    for source in sources:
