#!/usr/bin/env python3
"""Conversational reply cutover — flip DEEK_CONVERSATIONAL_REPLY_SHADOW to false.

Scheduled one-shot via cron for 2026-05-06 09:00 UTC. Checks that
the normaliser has run enough to be worth surfacing, then:

  1. Updates /opt/nbne/deek/deploy/.env — DEEK_CONVERSATIONAL_REPLY_SHADOW=false
  2. Restarts deek-api so the env takes effect
  3. Writes a cutover record to data/conversational_cutover.jsonl

Mirrors scripts/impressions_cutover.py and scripts/crosslink_cutover.py.

Safety gates:
    - At least 20 debug rows logged in conversational_reply_shadow
    - Span of at least 3 days between first and last row
    - Env file exists and is writable
    - deek-api container is running

Usage:
    python scripts/conversational_cutover.py              # safe mode
    python scripts/conversational_cutover.py --dry-run    # report only
    python scripts/conversational_cutover.py --force      # skip gates
"""
...
CUTOVER_LOG = Path(os.getenv(
    'DEEK_CONVERSATIONAL_CUTOVER_LOG',
    str(REPO_ROOT / 'data' / 'conversational_cutover.jsonl'),
))
...
log = logging.getLogger('conversational_cutover')


def read_shadow_stats() -> dict:
    """Count rows + span in cairn_intel.conversational_reply_shadow."""
    import psycopg2
...
def flip_env_file(env_file: Path) -> bool:
    ...
    try:
        ...
        return False
    key = 'DEEK_CONVERSATIONAL_REPLY_SHADOW'
    wanted = f'{key}=false\n'
    out: list[str] = []
    replaced = False
    for line in lines:
    ...
    if not replaced:
        if out and not out[-1].endswith('\n'):
            out.append('\n')
        out.append('\n# Conversational reply — cutover (auto-applied)\n')
        out.append(wanted)
    tmp = env_file.with_suffix(env_file.suffix + '.convcut-tmp')
    try:
...
def main() -> int:
    ...
    log.info('--- conversational shadow stats ---')
    ...
    record['cutover_applied'] = True
    write_cutover_record(record)
    log.info('CUTOVER COMPLETE — DEEK_CONVERSATIONAL_REPLY_SHADOW=false')
    return 0
