...
# Cairn Context Endpoint Definitions
...
Cairn is the memory layer that sits above all modules.
...
Once all three endpoints are live, Cairn assembles them into a single business
...
**Authentication**: All context endpoints use a shared `CAIRN_API_KEY` environment
variable. Set in each module's `.env`. Cairn passes this as a Bearer token.

**Cadence**: Cairn polls context endpoints:
...
**Graceful degradation**: If a module endpoint is unreachable, Cairn uses the
...
**Module**: Cairn internal (not a separate app — built into Cairn API)
...
```
GET /api/cairn/context  (internal — Cairn queries itself)
```
